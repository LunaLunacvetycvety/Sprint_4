...
class TestHeaderOrder:
    ...
    def test_header_order(self, driver, order_button):
        ...
        assert modal_title == "Заказ оформлен", 'Текст заголовка должен быть равен "Заказ оформлен"'
    ...
    def test_link_yandex_logo(self, driver):
        ...
        assert order_page.current_url() == data.dzen_url, 'Полученный урл должен быть урлом страницы дзен'
    ...
    def test_link_scooter_logo(self, driver):
        ...
        assert order_page.current_url() == data.base_url, 'При нажатии на логотип должна открыться главная страница'
